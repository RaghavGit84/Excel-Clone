...
$(document).ready(function () {
    ...
    for(let i = 1; i<=100; i++) {
        ...
        for(let j = 1; j<=100; j++) {
            ...
            let column = $(`<div class="input-cell" contenteditable="false" id = "row-${i}-column-${j}" data = "code-${colCode}"></div>`);
            row.append(column);
        }
    ...
    $(".input-cell").click(function(e) {
        ...
        if(e.ctrlKey) {
            ...
            if(rowId > 1){
                let topCellSelected = $(`#row-${rowId-1}-column-${colId}`).hasClass("selected");  //?????
                if(topCellSelected) {
                    $(this).addClass("top-cell-selected");
                    $(`#row-${rowId-1}-column-${colId}`).addClass("bottom-cell-selected");
                }
            }

            if(rowId < 100){
                let bottomCellSelected = $(`#row-${rowId+1}-column-${colId}`).hasClass("selected"); //?????
                if(bottomCellSelected) {
                    $(this).addClass("bottom-cell-selected");
                    $(`#row-${rowId+1}-column-${colId}`).addClass("top-cell-selected");
                }
            }

            if(colId > 1){
                let leftCellSelected = $(`#row-${rowId}-column-${colId-1}`).hasClass("selected");    
                if(leftCellSelected) {
                    $(this).addClass("left-cell-selected");
                    $(`#row-${rowId}-column-${colId-1}`).addClass("right-cell-selected");
                }
            }

            if(colId < 100){
                let rightCellSelected = $(`#row-${rowId}-column-${colId+1}`).hasClass("selected");    
                if(rightCellSelected) {
                    $(this).addClass("right-cell-selected");
                    $(`#row-${rowId}-column-${colId+1}`).addClass("left-cell-selected");
                }
            }
        }
...
function emptySheet() {
    ...
    for(let i of Object.keys(sheetInfo)) {
        for(let j of Object.keys(sheetInfo[i])) {
            $(`#row-${i}-column-${j}`).text("");
            $(`#row-${i}-column-${j}`).css("background-color", "#ffffff");
            $(`#row-${i}-column-${j}`).css("color", "#000000");
            $(`#row-${i}-column-${j}`).css("text-align", "left");
            $(`#row-${i}-column-${j}`).css("font-weight", "");
            $(`#row-${i}-column-${j}`).css("font-style", "");
            $(`#row-${i}-column-${j}`).css("text-decoration", "");
            $(`#row-${i}-column-${j}`).css("font-family", "Noto Sans");
            $(`#row-${i}-column-${j}`).css("font-size", "14px");
...
function loadSheet() {
    ...
    for(let i of Object.keys(sheetInfo)) {
        for(let j of Object.keys(sheetInfo[i])) {
            ...
            $(`#row-${i}-column-${j}`).text(cellInfo["text"]);
            $(`#row-${i}-column-${j}`).css("background-color", cellInfo["background-color"]);
            $(`#row-${i}-column-${j}`).css("color", cellInfo["color"]);
            $(`#row-${i}-column-${j}`).css("text-align", cellInfo["text-align"]);
            $(`#row-${i}-column-${j}`).css("font-weight", cellInfo["font-weight"]);
            $(`#row-${i}-column-${j}`).css("font-style", cellInfo["font-style"]);
            $(`#row-${i}-column-${j}`).css("text-decoration", cellInfo["text-decoration"]);
            $(`#row-${i}-column-${j}`).css("font-family", cellInfo["font-family"]);
            $(`#row-${i}-column-${j}`).css("font-size", cellInfo["font-size"]);
        }
    }
}
...
function addSheetEvents() {
    ...
    $(".sheet-tab.selected").contextmenu(function(e) {
        ...
        if($(".sheet-options-modal").length == 0) {
            $(".containers").append(`<div class="sheet-options-modal">
                                        <div class="sheet-rename">Rename</div>
                                        <div class="sheet-delete">Delete</div>
                                    </div>`);
            
            $(".sheet-rename").click(function() {
                $(".containers").append(`<div class="sheet-rename-modal">
